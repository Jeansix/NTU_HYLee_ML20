...
class Configurations(object):
    def __init__(self):
        ...
        self.data_dir = 'cmn-eng/'
        self.train_dir = 'cmn-eng/training.txt'
        self.val_dir = 'cmn-eng/validation.txt'
        self.test_dir = 'cmn-eng/testing.txt'
...
if __name__ == '__main__':
    ...
    data_process = Preprocess(config.data_dir, config.embed_dim, config.max_seq_len)
    ...
    train(model, optimizer, train_loader, val_loader, test_loader, loss_func, config.epochs, config.device,
          config.model_dir, data_process.int2word_cn, data_process.int2word_en)

    best_model = load_model(model, path=config.model_dir)
    loss_test, bleu_test, result = test(model, test_loader, loss_func, config.device, data_process.int2word_cn,
                                        data_process.int2word_en)
